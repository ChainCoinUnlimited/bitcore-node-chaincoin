...
var BitcoinRPC = require('bitcoind-rpc-chaincoin');
...
/**
 * Provides a friendly event driven API to chaincoind in Node.js. Manages starting and
 * stopping chaincoind as a child process for application support, as well as connecting
 * to multiple chaincoind processes for server infrastructure. Results are cached in an
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in chaincoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in chaincoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in chaincoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './chaincoin.conf');
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of bitcore-node-chaincoin. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bitcore-node-chaincoin. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bitcore-node-chaincoin. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that chaincoind is undergoing a reindex. ' +
             ...
             'of bitcore-node-chaincoin services will start.');
    node._reindex = true;
  }
};
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Chaincoin Daemon Ready');
          callback();
        });
      });
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/chaincoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/chaincoin.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Chaincoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Chaincoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/chaincoind.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned chaincoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean chaincoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting chaincoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Chaincoin process unexpectedly exited with code:', code);
        log.warn('Restarting chaincoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Chaincoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn chaincoind.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to chaincoind.'));
    }
 ...
 * @param {Object} txid - A chaincoin transaction id
 * @param {Function} callback
 */
...
Bitcoin.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	...
	if(nbOfBlockToFetch){
		if(nbOfBlockToFetch.constructor.name === "String"){
			if(!isNaN(parseInt(nbOfBlockToFetch))){
	...
	function queryHeader(err, blockhash) {
		...
		self._tryAllClients(function(client, done) {
			var hash = blockhash;
			var setInHeaders = function(_hash, _fetchNb, _headers){
				_fetchNb--;
				self.getBlockHeader(_hash, function(err, res){
					if (err) {
						hasErr=true;
						errMsg= err;
					}else if(res && res.hasOwnProperty('nextHash')){
						if(res.nextHash !== undefined){
							_headers.push(res);
							_hash = res.nextHash;
							if(_fetchNb<1){
								return done(null, _headers);
							}
							else{
								_headers = setInHeaders(_hash, _fetchNb, _headers);
							}
						}else{
							_headers.push(res);
							return done(null, _headers);
			...
			setInHeaders(hash, _toFetch, headers);
		}, callback);
	}
...
Bitcoin.prototype.getMNList = function(callback){
  ...
  //DEACTIVATED because of slow response from chaincoind
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('chaincoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('chaincoind process did not exit'));
      }
