...
`bitcore-node-chaincoin@2.1.1` to `bitcore-node-chaincoin@3.0.0`
...
```json
{
  ...
  "servicesConfig": {
    "bitcoind": {
      "spawn": {
        "datadir": "/home/<username>/.bitcoin",
        "exec": "/home/<username>/bitcore-node-chaincoin/bin/bitcoind"
      }
    }
  }
}
...
**Important**: Once changes have been made you'll also need to add the `reindex=1` option **only for the first startup** to regenerate the indexes. Once this is complete you should be able to remove the `bitcore-node-chaincoin.db` directory with the old indexes.
